--[[
BY : rnnni
Channel Files : https://t.me/rnnni
]]
local function keko_GeSoN(data)
    ...
    local var = false
    local creator = database:sismember('GeSoN:'..bot_id..'creator:'..chat_id, user_id) 
    local admin = database:sismember('GeSoN:'..bot_id..'admins:', user_id)
    if creator then var = true end
    if admin then var = true end
    for k,v in pairs(sudo_users) do
    if user_id == v then var = true end end
    local keko_add_sudo = redis:get('GeSoN:'..bot_id..'sudoo'..user_id..'')
    ...
    dl_cb, nil)
    database:set("GeSoN:get:hms:msg:"..bot_id..msg.chat_id_..msg.id_..t2.id_,keko_ts[1])
    function bot_id_get(t11,t22)
    bot_username = (t22.username_ or "GeSoNbot")
    ...
    kekol = database:get("GeSoN:get:hms:msg:"..bot_id..keko_ts[1]..keko_ts[2])
    if kekol then 
    ...
    database:set("GeSoN:get:hms:gr:"..bot_id..msg.chat_id_,"GeSoN")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل اهمس", 1, 'html')
    end 
    if text and text == "تفعيل اهمس" then 
    database:del("GeSoN:get:hms:gr:"..bot_id..msg.chat_id_)
